...
/**
 * Main function: Optimize route and open in Google Maps
 * @param {Array} requests - Array of requests with lat/lng
 * @param {Object} driverLocation - Driver's current location (optional)
 * @returns {Object} { url, distance, waypoints }
 */
export const openOptimizedRoute = (requests, driverLocation = null) => {
    ...
    const waypoints = validRequests.map(r => ({
        ...
        name: r.client_name || r.users?.name || 'Nepoznat klijent',
        address: r.address || r.users?.address || ''
    }));
