...
-- Description: Allows managers/company_admin to permanently delete users
...
BEGIN
    ...
    SELECT role, company_code INTO v_target_role, v_target_company_code
    FROM public.users
    ...
    -- Get the auth.users ID (might be same as public.users id, or linked via email)
    -- First try direct ID match
    SELECT id INTO v_target_auth_id FROM auth.users WHERE id = p_target_user_id;
...
-- Add comment
COMMENT ON FUNCTION public.delete_user_permanently IS 'Permanently deletes a user from both public.users and auth.users tables.';
