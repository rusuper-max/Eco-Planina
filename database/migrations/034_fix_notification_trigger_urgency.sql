-- Migration: Fix Notification Trigger Column (Urgency)
-- Description: Fixes bug where 'preferred_date' was used instead of 'urgency' column.
...
BEGIN
    ...
    -- Check if urgent (use 'urgency' column, assume 'urgent' or 'high' string value, or check logic)
    -- In DataContext, we see 'urgency' being passed. The frontend typically sends 'urgent' or 'normal'.
    v_is_urgent := (NEW.urgency = 'urgent');
